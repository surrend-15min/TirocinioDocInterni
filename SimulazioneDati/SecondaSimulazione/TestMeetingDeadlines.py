...
activity_ids = {
    ...
    "Invio fattura cliente": 14,
    "Pagamento fattura cliente ricevuto": 15,
    "Consegna": 16
}
...
activity_durations = {
    ...
    "Invio fattura cliente": {'minutes': (15, 60)},
    "Pagamento fattura cliente ricevuto": {'days': (1, 10)},
    "Consegna": {'days': (1, 10)}
}
...
def generate_cycle(product_id, start_time):
    ...
    timestamp = add_activity(data, "Invio fattura cliente", timestamp, product)

    if random.choice([True, False]):
        timestamp = add_activity(data, "Pagamento fattura cliente ricevuto", timestamp, product)
    
    timestamp = add_activity(data, "Consegna", timestamp, product)
    
    if not any(activity[1] == "Pagamento fattura cliente ricevuto" for activity in data):
        timestamp = add_activity(data, "Pagamento fattura cliente ricevuto", timestamp, product)
...
df.to_csv("test_meeting_deadlines.csv", index=False)
